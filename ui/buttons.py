...
class QueueClearButton(Button):
    ...
    async def callback(self, interaction):
        ...
        # 問使用者是否清空播放清單
        from ui.views import ConfirmClearQueueView
...
class QueueControlButton(Button):
    ...
    async def callback(self, interaction):
        ...
        if self.action == "play":
            ...
            from ui.views import PlaySelectionView
            ...
            from ui.views import QueueRemoveView
...
class InitAddButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from ui.views import InitAddModal
...
class InitNextButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer()
        from utils.initiative import next_turn, get_tracker_display, save_tracker
        from ui.views import InitiativeTrackerView
...
class InitRemoveButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        from ui.views import InitRemoveView
...
class InitEndButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from ui.views import InitEndConfirmView
...
class InitStatsButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        from ui.views import InitCharacterSelectView, InitStatsModalWithName
...
class InitHPButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from utils.initiative import get_entry_names, get_selected_character
        from ui.views import InitCharacterSelectView, InitHPModalWithName
...
class InitStatusButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        from ui.views import InitStatusBatchEditModal
        ...
        if selected:
            entry = get_entry(channel_id, selected)
            if entry:
                current_status = entry.get("status_effects", {})
                await interaction.response.send_modal(InitStatusBatchEditModal(self.ctx, selected, current_status))
            else:
                await interaction.response.send_message("❌ 找不到鎖定角色的資料", ephemeral=True)
            return
        ...
        class InitStatusSelect(discord.ui.Select):
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                name = self.values[0]
                entry = get_entry(str(self.ctx.channel.id), name)
                if entry:
                    current_status = entry.get("status_effects", {})
                    await interaction.response.send_modal(InitStatusBatchEditModal(self.ctx, name, current_status))
        ...
        await interaction.response.send_message("✨ 選擇要編輯狀態的角色：", view=InitStatusSelectView(self.ctx, names), ephemeral=True)
...
class InitFavDiceEditButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from ui.views import InitFavDiceActionSelectView
...
class InitFavDiceRollButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from utils.initiative import get_entry_names, get_selected_character, get_favorite_dice_names
        from ui.views import InitCharacterSelectView, InitFavDiceRollSelectView
...
class InitEditButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        from utils.initiative import get_entry_names, get_selected_character
        from ui.views import InitCharacterSelectView, InitEditInitiativeModal
...
class InitUnifiedEditButton(Button):
    """整合式編輯按鈕 (Demo)"""
    def __init__(self, ctx):
        super().__init__(label="📝 整合編輯", style=discord.ButtonStyle.secondary, row=1)
        self.ctx = ctx
    
    async def callback(self, interaction: discord.Interaction):
        from utils.initiative import get_selected_character, get_entry
        from ui.views import InitCharacterSelectView, InitUnifiedEditModal
        ...
        if selected:
            entry = get_entry(channel_id, selected)
            if entry:
                await interaction.response.send_modal(InitUnifiedEditModal(
                    self.ctx, 
                    selected, 
                    hp=entry.get("hp"),
        ...
        await interaction.response.send_message("📝 選擇要編輯的角色：", view=InitUnifiedSelectView(self.ctx, names), ephemeral=True)

class InitSaveCharButton(Button):
    def __init__(self, ctx):
        super().__init__(label="💾 保存角色", style=discord.ButtonStyle.secondary, row=2)
        self.ctx = ctx
    
    async def callback(self, interaction: discord.Interaction):
        from utils.initiative import get_selected_character, get_entry_names
        from ui.views import InitSaveSelectionView
        ...
        if selected:
            ...
            await interaction.response.send_message(f"💾 保存 **{selected}** 到全域庫：\n請選擇要保存的項目 (同名將覆蓋)", view=view, ephemeral=True)
        else:
            # 未鎖定，先選人
            names = get_entry_names(channel_id)
            if not names:
            ...
            class SaveSelect(discord.ui.Select):
                ...
                async def callback(self, interaction: discord.Interaction):
                    ...
                    await interaction.response.send_message(f"💾 保存 **{name}** 到全域庫：\n請選擇要保存的項目 (同名將覆蓋)", view=view, ephemeral=True)

            await interaction.response.send_message("💾 選擇要保存的角色：", view=SaveSelectView(self.ctx, names), ephemeral=True)

class InitLoadCharButton(Button):
    def __init__(self, ctx):
        super().__init__(label="📂 導入角色", style=discord.ButtonStyle.secondary, row=2)
        self.ctx = ctx
        
    async def callback(self, interaction: discord.Interaction):
        from utils.character_storage import get_all_names
        from ui.views import InitLoadSelectionView
        
        names = await get_all_names()
        if not names:
            await interaction.response.send_message("📂 全域角色庫是空的！", ephemeral=True)
            return
            
        view = InitLoadSelectionView(self.ctx, names)
        await interaction.response.send_message("📂 選擇要導入的角色：", view=view, ephemeral=True)

...
from ui.buttons import (NextSongButton, PrevSongButton, PauseResumeButton, PlaybackModeButton,
...
class InitiativeTrackerView(View):
    ...
    def setup_ui(self):
        from ui.buttons import (InitAddButton, InitNextButton, InitPrevButton, InitRemoveButton, 
                            ...
                            InitUnifiedEditButton, InitSaveCharButton, InitLoadCharButton)
...
class InitRemoveView(View):
    def __init__(self, ctx, names: list):
        super().__init__(timeout=60)
        self.ctx = ctx
        from ui.buttons import InitRemoveSelectButton, InitAddButton
        if not names:
...
class InitFavDiceActionSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        action = self.values[0]
        from utils.initiative import get_selected_character, get_entry_names, get_favorite_dice_names
...
class FavoriteDiceOverviewView(View):
    def __init__(self, ctx):
        super().__init__(timeout=None)
        self.ctx = ctx
        from ui.buttons import RerollAllInitiativeButton, QuickDiceButton
        from utils.shared_state import get_tracker
        from utils.initiative import get_selected_character
...
async def refresh_tracker_view(ctx):
    ...
    from utils.initiative import get_tracker_display, get_favorite_dice_display
...
class InitUnifiedEditModal(discord.ui.Modal):
    ...
    def __init__(self, ctx, character_name: str, hp=None, elements=None, atk=None, def_=None, initiative=None):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            new_elements = int(self.elements_input.value) if self.elements_input.value.strip() else None
            new_atk = int(self.atk_input.value) if self.atk_input.value.strip() else None
            new_def = int(self.def_input.value) if self.def_input.value.strip() else None
            
            if any(x is not None for x in [new_hp, new_elements, new_atk, new_def]):
                success = set_stats(self.channel_id, self.character_name, hp=new_hp, elements=new_elements, atk=new_atk, def_=new_def)
                if success:
                    updates.append("基礎數值")
                else:
        ...
        if updates:
            ...
            await refresh_tracker_view(self.ctx)
             ...
             await interaction.response.send_message(f"❌ 更新失敗: {'; '.join(error_msgs)}", ephemeral=True)
        else:
...
class InitStatusBatchEditModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新
        success = set_all_status(self.channel_id, self.character_name, new_status)
        
        if success:
            count = len(new_status)
            await interaction.response.send_message(f"✅ **{self.character_name}** 狀態已更新 (共 {count} 項)", ephemeral=True)
            await refresh_tracker_view(self.ctx)
        else:
            await interaction.response.send_message("❌ 更新失敗 (找不到角色)", ephemeral=True)
...
class InitSaveSelect(discord.ui.Select):
    def __init__(self, char_name):
        options = [
            discord.SelectOption(label="基礎數值", value="stats", description="HP, 元素, ATK, DEF"),
            discord.SelectOption(label="常用骰", value="dice", description="所有常用骰設定"),
            discord.SelectOption(label="先攻公式", value="formula", description="上次使用的擲骰公式")
        ]
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        entry = get_entry(str(interaction.channel.id), self.char_name)
        ...
        await interaction.response.send_message(f"✅ **{self.char_name}** 已保存至全域庫！", ephemeral=True)
...
class InitLoadSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        success, result, detail = add_entry_with_roll(channel_id, formula, char_name)
        ...
        if stats:
            set_stats(channel_id, char_name, 
                      hp=stats.get("hp"), 
        ...
        if fav_dice:
            count = 0
            for dice_name, dice_formula in fav_dice.items():
                add_favorite_dice(channel_id, char_name, dice_name, dice_formula)
                count += 1
